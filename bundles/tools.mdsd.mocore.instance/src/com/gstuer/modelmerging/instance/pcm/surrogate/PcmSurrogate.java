    ...
    public Set<Replaceable> replace(Replaceable original, Replaceable replacement) {
        ...
        for (Replaceable predecessor : this.replaceables) {
            if (predecessor.canReplace(original)) {
                /*
                 * If a predecessor needs replacement, create successor of obsolete replaceable.
                 * Then delete obsolete replaceable from model and return successor as
                 * implication for further merging. The model cannot add the successor itself
                 * because it might have consequences for other replaceables in the model.
                 */
